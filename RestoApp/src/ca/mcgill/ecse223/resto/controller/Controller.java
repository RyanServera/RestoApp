...
public class Controller {
   ...
   public static Table updateTable(Table aTable, int newNumber, int numberOfSeats) throws InvalidInputException {
	...
	public static void reserveTable(Date date, Time time, int numberInParty, String contactName, String contactEmailAddress, String contactPhoneNumber, List<Table> tables) throws InvalidInputException 
	{
		...
		if (date && time)  // logic needs to be implemented
		{
			error = "Date/Time is in the past.";
		}
		if (numberInParty < 0 ) 
		{
			error = "Please enter a positive number of people.";
		}
		if (error.length() > 0) 
		{
			throw new InvalidInputException(error.trim());
		...
		try 
		{
			...
			for (Table table : tables) 
			{
				...
				{
					if (reservation.doesOverlap()) // function needs to be added to reservation class via umple, not by editing autogenerated class
					{
						throw new InvalidInputException("There exists a conflicting reservation.");
					}
				}
			}
			if (seatCapacity < numberInParty)
			{
				throw new InvalidInputException("There are not enough seats to sit your party.");
			}
			Table[] tablesArray = (Table[]) tables.toArray(new Table[tables.size()]);
			
			Reservation res = new Reservation(date, time, numberInParty, contactName, contactEmailAddress, contactPhoneNumber, r, tablesArray);
			RestoApplication.save();
		}
